...
__all__ = ["ReIDBaseModel", "TeacherStudentNetwork",
            "build_model", "build_gan_model","build_transformer"]
...
class ReIDBaseModel(nn.Module):
    ...
    def forward(self, x, return_featuremaps=False, protos=False):
        try:
            batch_size = x.size(0)
        except:
            batch_size = 1
        ...
        #     # with torch.no_grad():
        #     for i in range (len(self.classifiers_old)):
        #         classifier_old = self.classifiers_old[i]
        #         if isinstance(feat, list):
        #             prob = [classifier_old(f) for f in feat]
        #         else:
        #             prob = classifier_old(feat)
        #         results["prob_"+str(i)] = prob
...
class TeacherStudentNetwork(nn.Module):
    ...
    def forward(self, x, return_featuremaps=False, protos=False):
        ...
        results = self.net(x, return_featuremaps, protos)
...
def build_model(
    cfg, num_classes,classifiers_old, init=None,
):
    """
...
def build_transformer(cfg, name, *args, **kwargs):
    """
