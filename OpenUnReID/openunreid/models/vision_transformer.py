...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, pretrained_path='',hw_ratio=1, conv_stem=False):
        ...
        # Classifier head
        self.fc = nn.Linear(embed_dim, num_classes, bias=False) if num_classes > 0 else nn.Identity()
    ...
    def initialize_centers(self, centers, labels):
        if self.num_classes > 0:
            ...
            ].copy_(centers.to(self.fc.weight.device))
        else:
            warnings.warn(
    ...
    def forward(self, x, return_featuremaps=False, protos=False):
        ...
        if not self.training:
            if protos:
                ...
                return bn_x
        if self.num_classes > 0:
            cls_score = self.fc(bn_x)
            results["prob"] = cls_score
