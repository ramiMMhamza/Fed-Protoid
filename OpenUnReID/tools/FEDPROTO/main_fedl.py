...
class MMTRunner(BaseRunner):
    def train_step(self, iter, batch, batch_protos=None, batch_target=None):
        ...
        len_ = int(len(data["id"]))
        
        
        inputs_1 = data["img"][0].cuda()
        if batch_target is not None:
            inputs_target = data_target["img"][0].cuda()
        targets = data["id"].cuda()
        if batch_target is not None:
            inputs_mo1 = torch.cat((inputs_1,inputs_target))
        else:
        ...
        if batch_target is not None:
            # results_1["feat"], results_1_mean["feat"] = results_1["feat"][:len_], results_1_target["feat"][:len_]
            # results_1["prob"], results_1_mean["prob"] = results_1["prob"][:len_], results_1_target["prob"][:len_]
            # results_1["pooling"], results_1_mean["pooling"] = results_1["pooling"][:len_], results_1_target["pooling"][:len_]
            results_target_features = results_1["feat"][len_:]
...
def main():
    ...
    train_loader_target, train_sets_target = build_train_dataloader(cfg, n_tasks, 0 ,[False], joint=False,for_kd=True)
