...
from openunreid.models import build_model, build_transformer
...
def add_model(model_1, model_2, w_1, w_2):
    ...
    with torch.no_grad():
        for name1, param1 in params1:
            if name1 in dict_params2 and ("clas" not in name1) and ("fc" not in name1):
...
class MMTRunner(BaseRunner):
    def train_step(self, iter, batch, batch_protos=None):
...
def parge_config():
    ...
    try:
        ...
        cfg_from_yaml_file("./cvlab-federated-phd/OpenUnReID/tools/FEDTRANSREID/config_s.yaml", cfg_s)
    except:
        try: 
            cfg_from_yaml_file("./fedl/data/OpenUnReID/tools/FEDTRANSREID/config_s.yaml", cfg_s)
        except:
            cfg_from_yaml_file("/exp/OpenUnReID/tools/FEDTRANSREID/config_s.yaml", cfg_s)
...
def main():
    ...
    logger = Logger(str(cfg.work_dir) + "/log_"+timestr+".txt", debug=False)
    sys.stdout = logger
    print("==========\nArgs:{}\n==========".format(args))
    ...
    for j in range (len(num_classes_)):
        if j == 0:
            # build model no.1
            model_1 = build_transformer(cfg_s, cfg_s.TRANSREID.arch,img_size=(cfg_s.DATA.height,cfg_s.DATA.width),drop_path_rate=0.3
                , pretrained_path = cfg.MODEL.source_pretrained ,hw_ratio=2, conv_stem=True, num_classes=num_classes_[j])
            model_1.cuda()
        else:
            # build model no.1
            model_1 = build_transformer(cfg, cfg.TRANSREID.arch,img_size=(cfg.DATA.height,cfg.DATA.width),drop_path_rate=0.3
                , pretrained_path = cfg.MODEL.source_pretrained ,hw_ratio=2, conv_stem=True, num_classes=num_classes_[j])
            model_1.cuda()
    ...
    runner_agg.resume_transformer(cfg.work_dir / "model_best_agg.pth")
    ...
    for idx, (loader, dataset) in enumerate(zip(loaders, datasets)):
        features, poolings = extract_features_protos(
            runner_agg.model[0], loader, dataset, with_path=False, prefix="Extract: ",
        )
    ...
    rounds = 800
    epochs_per_round = cfg.TRAIN.epochs
    cfg.TRAIN.epochs = runner_agg._start_epoch
    cfg_s.TRAIN.epochs = runner_agg._start_epoch
    for r in range(runner_agg._start_epoch,runner_agg._start_epoch+1): #rounds
        train_loader_s, train_sets_s = build_train_dataloader(cfg_s, n_tasks,
                0,[False], joint=False)
        train_loaders = [train_loader_s[0]]
        train_setss = [train_sets_s]
        ...
        # build runner
        cfg.task_id = -1
        cfg.TRAIN.epochs+=epochs_per_round
        cfg_s.TRAIN.epochs+=epochs_per_round
        runners = []
        clients_sizes = []
        print("start training at round : {}".format(r+1))
        for i in range(n_tasks+1):
            if i == 0 :
                runner = MMTRunner(
                cfg_s,
            ...
            # print(runner.train_sets)
            # runner.resume(cfg.work_dir / "model_best_agg.pth")
            runner.resume_transformer(cfg.work_dir / "model_best_agg.pth")
            try:
        ...
        #aggregate
        print("aggregating models after round {}".format(r))
        ...
        for i, (loader, query, gallery) in enumerate(zip(test_loaders, queries, galleries)):
            ...
            for idx in range(1):
                if cfg.TEST.datasets[i]=="msmt17" and i==0:
                    ...
                    print("map on "+l[i]+" domain : {}".format(mAP))
                if rank == 0:
                    writer.add_scalar("TEST_Map_Server_"+l[i]+"/per_epoch",float(mAP),r+epochs_per_round)
                    writer.add_scalar("TEST_Map_Server_"+l[i]+"/per_round",float(mAP),int(r/epochs_per_round)+1)
